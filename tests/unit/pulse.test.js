...
describe('Pulse Module', () => {
    ...
    describe('Declarative Events (Integration)', () => {
        ...
        it('should handle d-pulse="navigate" click', async () => {
            ...
            await vi.waitFor(() => {
                expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/hello'), expect.anything());
        ...
        it('should handle d-pulse="action" with data attributes', async () => {
            ...
            await vi.waitFor(() => expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/like'), expect.objectContaining({
                method: 'POST',
        ...
        it('should handle d-pulse="commit" on form submit', async () => {
            ...
            await vi.waitFor(() => expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/post'), expect.objectContaining({
                method: 'POST',
    ...
    describe('History & Navigation', () => {
        ...
        it('should handle popstate navigation', async () => {
            ...
            await vi.waitFor(() => {
                expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/previous'), expect.anything());
    ...
    describe('Edge Cases & API', () => {
        it('should handle commit with GET method', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/search?q=surf'), expect.objectContaining({
                method: 'GET'
            }));
        });
        ...
        it('should include cell state in action pulse', async () => {
            ...
            await vi.waitFor(() => {
                expect(global.fetch).toHaveBeenCalledWith('/api/user/update', expect.objectContaining({
                    method: 'POST',
        ...
        it('should handle errors in listeners during emit', () => {
             const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
             const brokenListener = () => { throw new Error('Boom'); };
             Pulse.on('before:pulse', brokenListener);
             
             Pulse.emit('before:pulse', {});
             
             expect(spy).toHaveBeenCalledWith(expect.stringContaining('Error in before:pulse listener'), expect.any(Error));
        ...
        it('should warn when applyPatches finds no target', async () => {
             ...
             expect(spy).toHaveBeenCalledWith(expect.stringContaining('Target not found for patch: #ghost'));
             spy.mockRestore();
        });
        ...
        it('should handle action pulse on anchor elements', async () => {
            ...
            await vi.waitFor(() => {
                expect(global.fetch).toHaveBeenCalledWith('/api/do', expect.objectContaining({
                    method: 'POST',
