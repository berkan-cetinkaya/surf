...
export async function createSprint(req, reply) {
    ...
    const html = await req.server.view('templates/examples/kanban/partials/backlog.ejs', { backlogBuckets });
...
export async function addTask(req, reply) {
    ...
    if (columnId === 'backlog') {
        ...
        const targetSprintId = sprintId || 'general';
        ...
        const html = await req.server.view('templates/examples/kanban/partials/backlog_content.ejs', { tasks: bucket.tasks, sprintId: bucket.sprintId });
    ...
    const html = await req.server.view('templates/examples/kanban/partials/column_content.ejs', { column, activeSprintId: sprintId });
    
    return createPatch()
        .addSurface(`#col-${columnId}`, html)
        .render();
}
...
export async function moveTask(req, reply) {
    ...
    if (fromCol === 'backlog') {
        ...
        const html = await req.server.view('templates/examples/kanban/partials/column_content.ejs', { column: sourceCol, activeSprintId: fromSprintId });
    ...
    if (toCol === 'backlog' || req.body.view === 'planning') {
        ...
        const html = await req.server.view('templates/examples/kanban/partials/column_content.ejs', { column: targetCol, activeSprintId: toSprintId });
...
export async function updateTaskDetails(req, reply) {
    ...
    if (tags !== undefined) {
        updates.tags = tags ? tags.split(',').map(t => t.trim()).filter(Boolean) : [];
    }
