...
export const Kanban = {
    ...
    createSprint(title) {
        ...
        const minOrder = db.prepare('SELECT MIN(sprint_order) as minOrder FROM sprints').get().minOrder || 0;
        db.prepare('INSERT INTO sprints (id, title, sprint_order) VALUES (?, ?, ?)').run(id, title, minOrder - 1);
    ...
    getBacklogBuckets() {
        ...
        sprints.forEach(sprint => {
            ...
            const tasks = db.prepare('SELECT * FROM tasks WHERE sprintId = ? ORDER BY created_at DESC')
                .all(sprint.id)
                .map(this._mapTask);
        ...
        const generalTasks = db.prepare('SELECT * FROM tasks WHERE columnId = ? AND sprintId IS NULL ORDER BY created_at DESC')
            .all('backlog')
            .map(this._mapTask);
    ...
    getColumn(colId) {
        ...
        // We'll update usecase to handle filtering.
         const tasks = db.prepare('SELECT * FROM tasks WHERE columnId = ? ORDER BY created_at DESC')
            .all(colId);
    ...
    addTask(colId, title, content = '', urgent = false, sprintId = null) {
        ...
        const priority = urgent ? 'critical' : priorities[Math.floor(Math.random() * priorities.length)];
        ...
        stmt.run(id, title, content, colId, sprintId, urgent ? 1 : 0, type, priority, comments, avatar, tags);
        
        return {
            id, title, content, columnId: colId, sprintId,
            urgent: Boolean(urgent), type, priority, assignee_avatar: avatar, 
            tags: tags ? JSON.parse(tags) : []
        };
    },

    updateTask(taskId, updates) {
        const { title, content, columnId, sprintId, urgent, type, priority, assignee_avatar, tags } = updates;
        ...
        if (columnId !== undefined) { query += 'columnId = ?, '; params.push(columnId); }
        if (sprintId !== undefined) { query += 'sprintId = ?, '; params.push(sprintId || null); }
        if (urgent !== undefined) { query += 'urgent = ?, '; params.push(urgent ? 1 : 0); }
        if (type !== undefined) { query += 'type = ?, '; params.push(type); }
        if (priority !== undefined) { query += 'priority = ?, '; params.push(priority); }
        if (assignee_avatar !== undefined) { query += 'assignee_avatar = ?, '; params.push(assignee_avatar); }
    ...
    getComments(taskId) {
        return db.prepare('SELECT * FROM comments WHERE task_id = ? ORDER BY created_at DESC').all(taskId);
    },

    addComment(taskId, content, author = { name: 'Me', avatar: 'https://i.pravatar.cc/150?u=me' }) {
        const id = Date.now().toString(); // Simple ID
        db.prepare('INSERT INTO comments (id, task_id, content, author_name, author_avatar) VALUES (?, ?, ?, ?, ?)').run(id, taskId, content, author.name, author.avatar);
    ...
    deleteComment(commentId) {
        ...
        if (result) {
            ...
            db.prepare('UPDATE tasks SET comments_count = comments_count - 1 WHERE id = ?').run(result.task_id);
